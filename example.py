...
print(np.shape(temp))
print(np.all(temp == mfcc_delta_second_order))
#print(temp)
#print(mfcc_delta_second_order)
...
print(np.shape(ubm_model.means_))
print(np.shape(ubm_model.predict_proba(ubm_feature)))

# def MAP_Estimation(data,model,iter):
...
#     temp_y,sr_temp = librosa.load(fname)
#     temp_mfcc = librosa.feature.mfcc(np.array(temp_y),sr_temp,hop_length=int(hop_length * sr),n_fft=int(n_fft*sr),n_mfcc=mfcc,dct_type=2)
...
#     temp_gmm_feature = preprocessing.scale(temp_gmm_feature)
...
for i in range(c):
    ...
    #temp_y=temp_y.reshape(np.shape(temp_y)[0],1)
...
# sc = SpectralClustering(n_clusters=clusters, affinity='cosine')
